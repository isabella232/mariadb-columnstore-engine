...
public:
    ...
    void useStoreStringMutex(bool b)
    {
        if (strings) strings->useStoreStringMutex(b);
    }
    bool useStoreStringMutex() const
    {
    ...
    void useUserDataMutex(bool b)
    {
        if (userDataStore) userDataStore->useUserDataMutex(b);
    }
    bool useUserDataMutex() const
    {
...
inline bool Row::equals(uint64_t val, uint32_t colIndex) const
{
    ...
    {
        case 1:
            return data[offsets[colIndex]] == val;

        case 2:
            return *((uint16_t*) &data[offsets[colIndex]]) == val;

        case 4:
            return *((uint32_t*) &data[offsets[colIndex]]) == val;
...
{
    int128_t *dst128Ptr = reinterpret_cast<int128_t*>(&data[offsets[colIndex]]);
...
/*
template<>
inline int128_t* Row::getBinaryField_offset<int128_t>(uint32_t offset) const
{
    int128_t *dst128Ptr = reinterpret_cast<int128_t*>(&data[offset]);
    MACRO_PTR_PTR_128(dst128Ptr, "=m", value, "m", "xmm0")
}*/
...
{
    MACRO_PTR_PTR_128(&x, "=m", &data[offsets[colIndex]], "m", "xmm0")
}
...
inline void Row::setUintField(uint64_t val, uint32_t colIndex)
{
    switch (len)
    {
        ...
        case 2:
            *((uint16_t*) &data[offsets[colIndex]]) = val;
            break;

        case 4:
            *((uint32_t*) &data[offsets[colIndex]]) = val;
            break;
...
inline void Row::setIntField(int64_t val, uint32_t colIndex)
{
    switch (len)
    {
        ...
        case 2:
            *((int16_t*) &data[offsets[colIndex]]) = val;
            break;
